...
def makeplatooninfo(dataset, simlen=50):
    ...
    vehlist, vehind, vehct = np.unique(dataset[:, 0], return_index=True,
                                       return_counts=True)  # get list of all vehicle IDs. we will operate on each vehicle.
    ...
    for z in range(len(vehlist)):  # first pass we can almost do everything
        ...
        mylen = len(lanedata)
        lanedata = np.append(lanedata,
                             lanedata)  # we need to make this into a np array because of the way checksequential works so we'll just repeat the column
        lanedata = lanedata.reshape((mylen, 2), order='F')
        unused, indjumps = checksequential(
            lanedata)  # indjumps returns indices of lanedata, lanedata itself are the indices of curveh.
        ...
        if np.all(indjumps == [0, 0]) or curveh[
            0, 6] == 0:  # special case where vehicle has no leaders will cause index error; we cannot simulate those vehicles
        ...
        t_n = int(curveh[lanedata[indjumps[
                                      0], 0], 1])  # simulated time is longest continuous episode with a leader, slice notation is lanedata[indjumps[0]]:lanedata[indjumps[1]]
        T_nm1 = int(curveh[lanedata[indjumps[1] - 1, 0], 1])

        if (
                T_nm1 - t_n) < simlen:  # if the simulated time is "too short" (less than 5 seconds) we will not simulate it (risk of overfitting/underdetermined problem)
            t_n = t_nstar
            T_nm1 = t_nstar

        platooninfo[i] = [t_nstar, t_n, T_nm1, T_n, [], curveh[t_n - t_nstar, 2], curveh[t_n - t_nstar, 3],
                          []]  # put in everything except for vehicle len and the follower info
    #        platooninfo[i] = [t_nstar, t_n, T_nm1, T_n, [], [0, []]]
    ...
    if len(leaders) == 0:
        print(
            'no leaders identified in data. data is either loaded incorrectly, or lead vehicles have circular dependency')
    ...
    for i in leaders:  # this will ensure that every vehicle designed as a leader has no leaders and no vehicle has a follower which is designated as a leader.
        ...
        for j in chklead:
            platooninfo[j][-1].remove(
                i)  # remove leader from the follower list of the leader's leaders; meaning the leader is not a follower
    ...
    for i in leadersc:  # initialize the simulation with the trajectory of the leaders
        if len(platooninfo[i][
                   -1]) == 0:  # in case there are any leaders without followers #probably don't need this if but meh we'll leave it in
            leaders.remove(i)
...
def c_metric(veh, platoon, T, platooninfo, meas, k=.9, metrictype='lead', depth=0, targetsdict={}):
    ...
    for i in leads:
        res += k * c_metric(i, platoon, getTimes(veh, i, T, targetsList), platooninfo, meas, k=k, metrictype=metrictype,
                            depth=depth + 1, targetsdict=targetsdict)
    return res
    ...
    if metrictype == 'veh':
        ...
        for i in range(len(platoonlist)):  # i is current platoon
            ...
            for j in range(len(platoonlist[i])):
                leaders = [k[0] for k in leadinfo[j]]
                leaders = set(leaders)
                leaders_after = leaders.intersection(
                    after)  # leaders_after are any leaders of i which are not yet calibrated
                ...
                for j in i:
                    if j:
                        for l in j[0]:
                            ...
                            temp += c_metric(l, platoonlist[j[1]], T, platooninfo, meas=meas, k=k,
                                             metrictype='follower')
            res.append(temp)
        return res
...
def is_pareto_efficient(costs,
                        return_mask=True):  # copy pasted from stack exchange. finds pareto front assuming lower is better
    """
        ...
        if my_mod:  # calculates headways as well to output flows
            ...
            for i in range(len(entry_data_for_lane)):
                ...
                if curlead == 0:
                    ...
                    curhd = meas[curlead][int(curtime - leadt_nstar), 2] - curleadlen - entry_data_for_lane[
                        i, 2] + vehlen
