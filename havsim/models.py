...
def check_coop_and_apply_sc(veh, ego_veh, veh_p, ego_safety, timeind):
    """For stochastic_lc_havsim."""
    unused, new_a = veh.get_cf(ego_veh, timeind)
    if new_a > ego_safety:
        if veh == getattr(ego_veh, ego_veh.coop_side_fol):
            if timeind == veh.next_t_ind:
                apply_coop(veh, ego_veh, new_a, veh_p)
            return
        elif veh.lead is not None:
            lead = veh.lead
...
def try_find_new_coop_sc(new_lcfol, veh, new_lcfol_a, veh_safety, timeind, coop_correction):
    """For stochastic lc_havsim."""
    if new_lcfol_a > veh_safety:
        maybe_add_new_coop_sc(new_lcfol, veh, new_lcfol, new_lcfol_a, timeind, coop_correction)
        return
    lead, lead_a = new_lcfol, new_lcfol_a
    for i in range(5):
        test_veh = lead.fol
        ...
        if test_veh_a > veh_safety:
            if lead.speed > veh.speed:
                maybe_add_new_coop_sc(test_veh, veh, new_lcfol, test_veh_a, timeind, coop_correction)
            break
        lead, lead_a = test_veh, test_veh_a


def maybe_add_new_coop_sc(test_veh, veh, new_lcfol, test_veh_a, timeind, coop_correction):
    """For stochastic lc_havsim"""
    if test_veh.is_coop is None:  # not currently cooperating
        coop_p = test_veh.lc2_parameters[5] + coop_correction
        if veh.npr.random() < coop_p:  # cooperation condition met
            ...
            test_veh.chk_disc = False
            test_veh.disc_endtime = timeind + test_veh.lc_parameters[7]
            if timeind == test_veh.next_t_ind:
                apply_coop(test_veh, veh, test_veh_a, test_veh.lc2_parameters)
...
def new_relaxation(veh, timeind, dt):
    ...
    if veh.lead is None:  # new lead is None -> reset relaxation
        ...
        return
    prev_lead = get_prev_lead(veh, timeind)
    ...
    relax_helper_vhd(rp[0], rp[1], 60, relax_amount_s, relax_amount_v, veh, timeind, dt)
...
def relax_helper_vhd(pos_r, neg_r, max_s, relax_amount_s, relax_amount_v, veh, timeind, dt):
    ...
    Args:
        ...
        relax_amount_s: relaxation amount for headway (float)
        relax_amount_v: relaxation amount for speed (float)
        veh: Vehicle to apply relaxation to
        timeind: time index
        dt: length of timestep
    """
    # rp = parameter, relaxamount_s = headway relaxation, _v = velocity relaxation
    rp = neg_r if relax_amount_v < 0 else pos_r
    relaxlen = int(np.ceil(rp/dt)) - 1
    if relaxlen <= 0:
        return

    relax_amount_s = max(min(relax_amount_s, max_s), -max_s)
    if veh.in_relax:  # case where relaxation is added to existing relax
        if timeind > veh.relax_start + len(veh.relax) - 1:  # edge case where veh is not actually in_relax
            veh.relaxmem.append((veh.relax, veh.relax_start))
            apply_normal_relaxation(veh, rp, relax_amount_s, relax_amount_v, timeind, dt, relaxlen)
        else:
            # calculate updated relax amount taking into account the current relax
            old_relax_s = veh.relax[timeind - veh.relax_start][0]
            relax_amount_s = max(min(relax_amount_s + old_relax_s, max_s), -max_s)
            relax_amount_s = relax_amount_s - old_relax_s
            tempdt = -dt / rp * relax_amount_s
            tempdt2 = -dt / rp * relax_amount_v
            curr = [(relax_amount_s + tempdt * i, relax_amount_v + tempdt2 * i) for i in range(1, relaxlen + 1)]
...
def default_parameters(truck_prob=0., stochasticity=True):
    ...
    if is_car:
        ...
        lc_parameters = [-11, -11, .3, .03, 0, 0, .1, 5, 100]
        lc2_parameters = [1.2, 2.5, 1, -1, 1, .5]
        ...
        lc2_parameters = [1.2, 2.5, 1, -.5, 1, .5]
...
def stochastic_lc_havsim(veh, lc_actions, lc_followers, timeind):
    ...
    is also not updated. Note that the cooperation relationships are always updated.
    """
    if veh.chk_disc:
        if veh.npr.random() > veh.lc_parameters[6]:
            if veh.has_coop:  # remove coop
    ...
    if veh.in_disc:  # discretionary update
        ...
        if incentive > p[2]:
            ...
            has_coop = veh.has_coop
            if not has_coop:
                try_find_new_coop_sc(new_lcfol, veh, new_lcfol_a, safety, timeind, 0)
            elif has_coop.lc_acc == 0:
                check_coop_and_apply_sc(has_coop, veh, has_coop.lc2_parameters, safety, timeind)
        ...
        if not has_coop:
            s1, s2 = veh.lc_urgency
            coop_correction = (veh.pos - s1)/(s2 - s1 + 1)
            try_find_new_coop_sc(new_lcfol, veh, new_lcfol_a, safety, timeind, coop_correction)
        elif has_coop.lc_acc == 0:
            check_coop_and_apply_sc(has_coop, veh, has_coop.lc2_parameters, safety, timeind)
