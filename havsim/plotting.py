...
def generate_confmat(sim_vehdict, vehid):
    """
    This creates a confusion matrix for lane changing for a single simulated vehicle.
    Args:
        sim_vehdict: dictionary of simulated vehicle data generated by havsim.calibration.deep_learning.
            generate_vehicle_data
        vehid: (float) of vehicle id
    Returns:
        confusion matrix (np.ndarray with shape (3,3)) for lane changing for a single vehicle
    """
    veh = sim_vehdict[vehid]
    pred = []
    for idx in range(veh.lc_actions.shape[0]):
        pred.append(np.random.choice([0, 1, 2], size=1, p=veh.lc_actions[idx]))
    pred = np.stack(pred).flatten()

    start, end = veh.start, veh.end
    start_sim, end_sim = veh.longest_lead_times

    true_lc = np.array(hc.deep_learning.generate_lane_data(veh))[start_sim - start:end_sim - start+1]
    confmat = np.zeros((3, 3))
    for idx_true in range(3):
        for idx_pred in range(3):
            confmat[idx_true, idx_pred] = np.sum((true_lc == idx_true) & (pred == idx_pred))
    return confmat

def generate_full_confmat(sim_vehdict):
    """
    This creates a confusion matrix for all simulated vehicles.
    Args:
        sim_vehdict: dictionary of simulated vehicle data generated by havsim.calibration.deep_learning.
            generate_vehicle_data
    Returns
        confusion matrix (np.ndarray with shape (3,3)) for lane changing
    """
    confmat = np.zeros((3, 3))
    for vehid in tqdm(sim_vehdict, total=len(sim_vehdict)):
        confmat += generate_confmat(sim_vehdict, vehid)
    return confmat

def plot_confmat(sim_vehdict, save=False, output_dir=None):
    """
    This plots the confusion matrix for all simulated vehicles
    Args:
        sim_vehdict: dictionary of simulated vehicle data generated by havsim.calibration.deep_learning.
            generate_vehicle_data
    """
    confmat = generate_full_confmat(sim_vehdict)
    plt.clf()
    sns.heatmap(confmat, annot=True)
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    if save and output_dir:
        plt.savefig(f'{output_dir}/lc_confmat.png')
    else:
        plt.show()
...
def plotTrajectoryProbs(vehdict, vehid, save=False, output_dir=None):
    ...
    if save:
        plt.savefig(f'{output_dir}/{vehid}.png')
    # else:
    #     plt.show()
...
def cferror(vehdict, sim_vehdict, vehid):
    """
    Computes CF error between true trajectories and simulated trajectories
    Params:
        vehdict: vehdict generated from havsim.helper.make_dataset
        sim_vehdict: vehdict generated from havsim.deep_learning.generate_vehicle_data
        vehid: vehid (float)
    Returns:
        np.ndarray of errors with shape (nt,)
    """
...
def plotCFErrorN(sim_vehdict, vehdict, output_dir, n=20, seed=42):
    """
    ...
    np.random.seed(seed)
    idxs = np.random.choice(list(sim_vehdict.keys()), n)
    for idx in idxs:
        plotCFError(cferror(vehdict, sim_vehdict, idx), idx, save=True, output_dir=output_dir)

def plotTrajectoriesProb(sim_vehdict, output_dir, n=20, seed=42):
    """
    ...
    idxs = np.random.choice(list(sim_vehdict.keys()), n)
    for idx in idxs:
        plotTrajectoryProbs(sim_vehdict, idx, save=True, output_dir=output_dir)
