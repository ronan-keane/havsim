...
class Lane:
    ...
    def connects(self, new_lane, connect_type, side=None, self_pos=None, new_lane_pos=None):
        """
        new_lane: new lane to be connected to
        connect_type: one of 'continue', 'exit' and 'merge'
        side: 'l' or 'r', if not specified a side, will infer it automatically
        self_pos: a tuple indicating the start/end position of the connection for the current lane
        new_lane_pos: a tuple indicating the start/end position of the connection for the new lane
        """
        if connect_type == 'exit':
            # For 'exit' type, the lane's roadlen attribute is not updated. The road's connect_to attribute is updated
            # already in the Road's connects() method. We don't update again here.
            self.events.append({'event': 'exit', 'pos': self.end})
        elif connect_type == 'continue':
            # For 'exit' type, the lane's roadlen attribute and the road's connect_to attribute are updated
            # already in the Road's connects() method. We don't update again here.

            # Update self road's connect_to
            self.events.append({'event': 'new lane', 'pos': self.end})
        else:
            assert connect_type == 'merge'
            if side == 'l':
                change_side = 'l_lc'
            elif side == 'r':
                change_side = 'r_lc'
                ...
                assert side is None
                change_side = 'l_lc' if self.laneind == 0 else 'r_lc'
            # Update self road's connect_to
            self.road.connect_to[new_lane.road.name] = (self_pos, 'merge', self.laneind, change_side, new_lane.road)
            assert isinstance(self_pos, tuple) and isinstance(new_lane_pos, tuple)
            # Update roadlen
            self.roadlen[new_lane.road.name] = new_lane_pos[0] - self_pos[0]
            new_lane.roadlen[self.road.name] = self_pos[0] - new_lane_pos[0]
            # Update lane events and connect_left/connect_right for both lanes
            if change_side == 'l_lc':
                connect_lane_left_right(new_lane, self, new_lane_pos, self_pos)
            else:
                connect_lane_left_right(self, new_lane, self_pos, new_lane_pos)
